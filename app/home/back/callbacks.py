...
def register_callbacks(dashapp):
    ...
    @dashapp.callback(Output('liste-tournois', 'children'),
        [Input('go-home-button', 'n_clicks')])
    def update_tournois(n_clicks):
        ...
        for i in range(len(df_tournois)):
            card = create_tournament_card(df_tournois.nom[i])
            #if i%5==0:
            #    children.append(dbc.Row([], no_gutters=True, justify='around', align = 'center', style={'height':'30px'}))
